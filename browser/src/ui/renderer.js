// unk Browser — tab + navigation logic (runs in the chrome UI renderer).
...
if (window.unk && window.unk.onRegions) {
  window.unk.onRegions(populateRegions);
}
if (window.unk && window.unk.vpnRegions) {
  window.unk.vpnRegions().then(populateRegions).catch(() => {});
}
...
vpnRegionSel.addEventListener('change', async () => {
  ...
  if (vpnOn) {
    try {
      const res = await window.unk.toggleVpn(true, vpnRegion);
      vpnOn = res.on;
    } catch {
      /* ignore */
    }
...
if (window.unk && window.unk.onBlocked) {
  window.unk.onBlocked((n) => {
