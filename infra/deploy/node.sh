#!/usr/bin/env bash
# unk — stand up a REMOTE VPN region node (WireGuard exit + control agent).
...
cp "$(dirname "$0")/../agent/unk-agent.service" /etc/systemd/system/unk-agent.service
systemctl daemon-reload
systemctl enable unk-agent >/dev/null 2>&1 || true
systemctl restart unk-agent
...
echo " (the array in /opt/unk/api/.env), then restart the API."
echo "============================================================"
cat <<EOF
{
  "id": "${REGION_ID}",
