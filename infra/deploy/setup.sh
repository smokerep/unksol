#!/usr/bin/env bash
# unk — one-shot backend deploy for Ubuntu 22.04 / 24.04 (run as root).
...
REPO="${REPO:-https://github.com/giupy997/anonrh.git}"
APP_DIR="${APP_DIR:-/opt/unk}"
# ==================================================
...
cp "$APP_DIR/infra/deploy/unk-api.service" /etc/systemd/system/unk-api.service
systemctl daemon-reload
systemctl enable unk-api >/dev/null 2>&1 || true
...
echo "   Logs:    journalctl -u unk-api -f"
