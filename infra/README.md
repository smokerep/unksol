...
```bash
cd /opt/unk && bash infra/deploy/vpn.sh
```
...
unk Search proxies a self-hosted [SearXNG](https://docs.searxng.org) instance — a privacy metasearch engine that aggregates 70+ sources with no tracking, ads, or logs. The backend enforces token-gating and forwards queries to it.
...
```bash
cd /opt/unk && bash infra/deploy/searxng.sh
```
...
unk calls SearXNG's JSON output, which is **off by default**. In `infra/searxng/settings.yml` add:
